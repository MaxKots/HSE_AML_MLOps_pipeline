...
def resolve_bundle_path(model_type: str) -> Path:
    ...
    exact_candidates = [
        benchmark_dir / f"samld_{model_type}_prepared_{model_type}_prepared_bundle.joblib",
    ...
    if benchmark_dir.exists():
        matches = sorted(benchmark_dir.glob(f"*samld*{model_type}*bundle*.joblib"))
        if matches:
            return matches[0]

    raise FileNotFoundError(
        f"Не найден bundle для SAML-D и модели {model_type}. "
        "Сначала запусти aml_benchmark_dag."
    )
...
def build_shap_for_model(
    ...
    print(f"SAML-D test rows: {len(test_df)}")
    ...
    csv_path = output_dir / f"samld_{model_type}_test_top{top_n}_shap_importance.csv"
    summary_dot_path = output_dir / f"samld_{model_type}_test_top{top_n}_shap_summary_dot.png"
    summary_bar_path = output_dir / f"samld_{model_type}_test_top{top_n}_shap_summary_bar.png"
    ...
    plt.title(
        f"SHAP summary: {model_type.upper()} на top-{top_n} объектов SAML-D"
    )
    plt.tight_layout()
    ...
    plt.title(
        f"SHAP importance: {model_type.upper()} на top-{top_n} объектов SAML-D"
    )
    plt.tight_layout()
...
def main() -> None:
    ...
    df = prepare_samld_dataset(
        path=settings.samld_dataset,
        max_rows=settings.samld_max_rows,
    )

    # Важно: список признаков проверяется здесь, чтобы быстро поймать ошибку подготовки.
    categorical_columns, numerical_columns = get_samld_feature_columns(df)
    print(
        f"SAML-D подготовлен: rows={len(df)}, "
    ...
    report_path = get_artifacts_dir() / "shap" / "samld_shap_report.csv"
